...
class RegisterUser(Resource):
    ...
    def post(self):
        dataUsername = request.form.get('name')
        ...
        if dataUsername and dataEmail and dataPassword:
            #
            dataModel = LoginModel(username=dataUsername,email=dataEmail, password=dataPassword)
            db.session.add(dataModel)
            db.session.commit()
            return make_response(jsonify({"msg":"Register berhasil"}), 200)
        return jsonify({"msg":"Name, Email and Password data cannot be empty"})    
...
class LoginUser(Resource):
    def post(self):
        ...
        queryUsername = [data.username for data in LoginModel.query.all()]
        queryEmail = [data.email for data in LoginModel.query.all()]
        queryPassword = [data.password for data in LoginModel.query.all()]
        if dataUsername in queryUsername and dataEmail in queryEmail and dataPassword in queryPassword:
            # login sukses & generate token auth
            token = jwt.encode(
               {
                   "name":queryUsername, "exp":datetime.datetime.utcnow() + datetime.timedelta(minutes=60)
...
if __name__ == "__main__":
    login.run(debug=True, port=8080)
